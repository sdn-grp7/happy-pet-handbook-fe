...
export function AdoptionPage() {
  ...
  const [showAdoptFormOnOpen, setShowAdoptFormOnOpen] = useState(false);
  ...
  const openPet = useCallback(
    (pet: PetListing, options?: { showAdoptForm?: boolean }) => {
      setSelected(pet);
      setShowAdoptFormOnOpen(options?.showAdoptForm ?? false);
  ...
  return (
    <>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {pet.status === "available" ? (
                      <button
                        ...
                        onClick={(event) => {
                          ...
                          openPet(pet, { showAdoptForm: true });
                        }}
                        className="inline-flex items-center justify-center rounded-full bg-primary px-3 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90"
                      >
                        {t("pet.submitRequest")}
                      </button>
                    ) : null}
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-full border border-border px-3 py-2 text-sm font-medium text-foreground transition hover:bg-muted"
                    >
      ...
      <PetDetailModal
        ...
        initialShowAdoptForm={showAdoptFormOnOpen}
      />
    </>
  );
}

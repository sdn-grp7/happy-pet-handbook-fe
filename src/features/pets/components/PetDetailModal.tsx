...
export function PetDetailModal({
  ...
  useEffect(() => {
    if (!open) return;
    setActiveImage(0);
    setShowAdoptForm(initialShowAdoptForm);
    setMessage("");
    setSubmitted(false);
    setSubmitting(false);
  }, [open, pet?.id, initialShowAdoptForm]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="grid min-h-0 flex-1 overflow-y-auto md:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)]">
          <div className="flex flex-col gap-3 bg-muted/40 p-4 sm:p-5">
            <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-card)]">
              <PetImage
                src={images[activeImage]}
                alt={pet.name}
                className="aspect-[4/5] w-full object-contain sm:aspect-[3/4]"
              />
            </div>
            {images.length > 1 && (
